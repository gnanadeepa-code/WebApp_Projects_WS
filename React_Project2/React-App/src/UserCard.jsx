...
export const UserCard = (props) => {
  ...
  //we can use, the use API in react to read values from context instead of useContext hook as below,since use API is not a hook it does not need to follow the rules of hooks like, 1.Hooks must be called at the top level of component 2.Can't call them inside conditions, loops or early returns 3.Every render has to call hooks in the same order
  ...
  return (
    <div>
      <h2>User Details</h2>
      <UserInfo {...props} />
      <ActionButton text="Save Details" onClick={handleSaveDetails} />{" "}
      {/* here the event handling is passed as props */}
      <h3>Using Context</h3>
      <p>Name: {user.name}</p>
      <p>ROle: {user.role}</p>
